...
var LayerSlider = {
	...
	updateSlideInterfaceItems: function() {
		...
		// Slide image
		var imgSrc = slideData.backgroundThumb ? slideData.backgroundThumb : slideData.background;
		jQuery('.ls-slide-image img').attr('src', imgSrc ? imgSrc : lsTrImgPath+'/not_set.png');
	},
	...
	updateLayerInterfaceItems: function(layerIndex) {
		...
		jQuery('.ls-layer-image img').attr('src', imgSrc ? imgSrc : lsTrImgPath+'/not_set.png');

		// Video poster preview
		imgSrc = layerData.posterThumb ? layerData.posterThumb : layerData.poster;
		jQuery('.ls-media-image img').attr('src', imgSrc ? imgSrc : lsTrImgPath+'/not_set.png');
	...
	startSlidePreview: function( sliderOptions ) {
		...
		jQuery.each(window.lsSliderData.layers, function(slideIndex, slideData) {
			...
			if( !tr2d  && !tr3d  && !tr2dcustom && !tr3dcustom  ) {
				layer.attr('data-ls', layer.attr('data-ls') + ' transition2d: all; transition3d: all; ');
			} else {
...
var LS_DataSource = {
	...
	buildLayer: function() {
		...
		var imageURL = !layerData.imageThumb ? lsTrImgPath+'/not_set.png' : layerData.imageThumb;
		$layer.find('input[name="image"]').next().find('img').attr('src', imageURL);

		// Set media image placeholder
		imageURL = !layerData.posterThumb ? lsTrImgPath+'/not_set.png' : layerData.posterThumb;
		$layer.find('input[name="poster"]').next().find('img').attr('src', imageURL);
...
jQuery(document).ready(function() {
	...
	// Clear uploads
	jQuery(document).on('click', '.ls-image a:not(.aviary)', function(e) {
		e.preventDefault();
		e.stopPropagation();
	...
	jQuery('#ls-layers').on('click', '.slide-image .ls-post-image', function(e) {
		...
		if( imageHolder.hasClass('ls-slide-image') ) {
			...
			imageHolder.find('img').attr('src', pluginPath+'admin/img/not_set.png');
